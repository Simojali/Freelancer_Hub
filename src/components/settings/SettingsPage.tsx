    ...
    <div className="space-y-8 max-w-xl">
      ...
      <div className="bg-card border border-border rounded-lg p-5">
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm text-muted-foreground w-24 shrink-0">Display currency</label>
      ...
      {/* Services */}
      <div className="bg-card border border-border rounded-lg p-5">
        <h2 className="text-sm font-semibold text-foreground mb-1">Services</h2>
        <p className="text-xs text-muted-foreground mb-4">Services appear in all dropdowns. The slug is stored in your data and cannot be changed after creation.</p>

        <div className="space-y-1 mb-4">
          {services.map(s => (
            <div key={s.id} className="flex items-center gap-3 px-3 py-2 rounded-md border border-border hover:border-border bg-muted/40">
              {editingId === s.id ? (
                <>
                  <input type="color" value={editColor} onChange={e => setEditColor(e.target.value)} className="w-7 h-7 rounded cursor-pointer border border-border shrink-0" />
                  <Input
                    value={editName}
                ...
                <>
                  ...
                  <span className="text-xs text-muted-foreground font-mono bg-muted px-1.5 py-0.5 rounded">{s.slug}</span>
                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-foreground" onClick={() => startEdit(s)}>
        ...
        <div className="border-t border-border pt-4">
          <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2">Add service</div>
          <div className="flex items-start gap-2">
            <input
              type="color"
              value={newColor}
              onChange={e => setNewColor(e.target.value)}
              className="w-9 h-9 rounded cursor-pointer border border-border shrink-0 mt-0.5"
            />
            <div className="flex-1">
              ...
              {newName && (
                <div className="text-xs text-muted-foreground mt-1">
                  Slug: <span className={`font-mono ${slugExists ? 'text-red-500' : 'text-muted-foreground'}`}>{newSlug || '—'}</span>
