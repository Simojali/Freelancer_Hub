...
export default function EarningsKpiCard({ earnings, currency }: Props) {
  ...
  if (prior === 0 && amount === 0) {
    trendNode = <span className="text-muted-foreground">no earnings {LABEL_FOR_PRIOR[period]}</span>
  } else if (prior === 0) {
    trendNode = (
      <>
      ...
      <CardContent className="p-5">
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2 min-w-0">
            <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide shrink-0">Earned</span>
